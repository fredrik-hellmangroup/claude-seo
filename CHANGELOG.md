...
- **requirements.txt**: Now persisted to `~/.copilot/skills/seo/` for user retry
...
### Changed
- Python dependencies now installed in a venv at `~/.copilot/skills/seo/.venv/` with `--user` fallback (#2)
